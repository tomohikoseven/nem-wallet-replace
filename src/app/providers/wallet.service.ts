...
// class
import { Contact, Endpoint, Account, Current, CurrentAccount, BIP32Data, NtyData, Common, Wallet } from '../class';
...
export class WalletService {
  @ViewChild('downloadWallet') downloadWallet: ElementRef;
  current: Current;
   ...
   */
  load(data:string, isNCC:boolean):boolean {
      if (!data) {
  ...
  login(common:Common, wallet:Wallet) {
      ...
      if(wallet.accounts[0].network === nem.model.network.data.mainnet.id && AppConstants.mainnetDisabled) {
          this._Alert.mainnetDisabled();
          return false;
      }
      // Check if mijinnet is disabled
      if(wallet.accounts[0].network === nem.model.network.data.mijin.id && AppConstants.mijinDisabled) {
   ...
   */
  useAccount(wallet, index:number) {
      if (!wallet) {
          this._Alert.noWalletToSet();
          return false;
      }
  ...
  deriveRemote(common, account) {
      ...
      return this._deriveRemote(common, _account, algo, network).then((data:BIP32Data) => {
   ...
   * @return {Promise} - A resolved promise with true if success, or a rejected promise
   */
  addAccount(common, label:string) {
  ...
  _deriveAccount(common, label, newAccountIndex) {
      ...
      return CryptoHelpers.generateBIP32Data(common.privateKey, common.password, newAccountIndex, this.network).then((data:BIP32Data) => {
  ...
  prepareDownload(wallet) {
      ...
      //console.log(this.downloadWallet);
      //this.downloadWallet.nativeElement.href = 'data:application/octet-stream,' + base64;
      //this.downloadWallet.nativeElement.download = wallet.name + '.wlt';

      let doc = <HTMLInputElement>document.getElementById('downloadWallet');
      console.log(doc);
      doc.setAttribute('href','data:application/octet-stream,' + base64);
      doc.setAttribute('download',wallet.name + '.wlt');
      return true;
  }
   ...
   */
  base64Encode(wallet):string {
      // Wallet object string to word array
      let wordArray = nem.crypto.js.enc.Utf8.parse(JSON.stringify(wallet)); 
